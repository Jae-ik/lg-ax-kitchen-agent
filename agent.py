...
# ─────────────────────────────────────────────────────────────────────
# 이 경로는 **제출 경로가 아니며 검증되지 않았다.** API 키가 없어 한 번도
# 돌려 보지 못했고, 그 사이 실행 층이 여러 번 바뀌었다.
#
# 현재 상태 (2026-09-25 확인)
#   · import 는 된다. 부르는 kitchen 함수도 전부 존재한다.
#   · 그러나 **레지스트리(skills/)를 쓰지 않는다** — kitchen 함수를 직접
#     감싼 도구를 따로 만든다. 그래서 제출 경로가 쓰는 판단들이 여기엔
#     하나도 없다: 여열 예측, 이상 감지, 중간 투입, 흡수량 계산, 익힘,
#     물 되돌리기, 기기 용량, 인분 환산.
#   · 즉 같은 주방을 다루지만 **같은 에이전트가 아니다.**
#
# 제출 경로(run_design.py)와 견주려면 레지스트리 스킬을 도구로 노출하도록
# 다시 써야 한다. 그 전까지 이 파일의 결과를 성능 근거로 쓰지 않는다.
# ─────────────────────────────────────────────────────────────────────
