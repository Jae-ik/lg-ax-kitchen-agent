...
class Skill:
    ...
    `requires` / `provides` 는 **이 스킬이 주방 도메인에서 맡는 자리**를 적어
    둔 것이고, 플래너는 이 값을 읽지 않는다 — 플래너가 보는 것은 도메인이
    만든 `Task` 의 선언이다. 같은 스킬이 도메인마다 다른 사실을 만들 수 있기
    때문이다(`converge` 는 조리기에서 'cooked', 건조기에서는 다른 것을 만든다).
    그래서 기기에 따라 달라지는 스킬은 여기를 비워 둔다.
    """
    name: str = ""
    description: str = ""          # 에이전트가 읽고 스킬을 고르는 근거
    # 클래스 속성이라 모든 인스턴스가 공유한다. 하위 클래스가 반드시
    # 덮어쓰므로 지금은 문제가 없지만, 안 덮어쓴 스킬이 생기면 서로의 값을
    # 보게 된다. 빈 값을 그대로 쓰지 말고 각 스킬이 자기 것을 적는다.
    input_schema: dict = {}
